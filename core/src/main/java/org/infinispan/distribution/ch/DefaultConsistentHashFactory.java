...
public class DefaultConsistentHashFactory implements ConsistentHashFactory<DefaultConsistentHash>, Serializable {
   ...
   protected void addPrimaryOwners(Builder builder) {
      ...
      swapPrimaryOwnersWithBackups(builder, minPrimarySegments, minPrimarySegments + 1);
      swapPrimaryOwnersWithBackups(builder, minPrimarySegments, minPrimarySegments);
      swapPrimaryOwnersWithBackups(builder, minPrimarySegments + 1, minPrimarySegments + 1);
      ...
      doAddPrimaryOwners(builder, minPrimarySegments, minPrimarySegments + 1, actualNumOwners);
      doAddPrimaryOwners(builder, minPrimarySegments, minPrimarySegments, actualNumOwners);
      doAddPrimaryOwners(builder, minPrimarySegments + 1, minPrimarySegments + 1, actualNumOwners);
      ...
      // for each segment.
      doAddPrimaryOwners(builder, minPrimarySegments, minPrimarySegments + 1, actualNumOwners + 1);
      doAddPrimaryOwners(builder, minPrimarySegments, minPrimarySegments, actualNumOwners + 1);
      doAddPrimaryOwners(builder, minPrimarySegments + 1, minPrimarySegments + 1, actualNumOwners + 1);
   }

   protected void doAddPrimaryOwners(Builder builder, int minSegments, int maxSegments, int maxOwners) {
   ...
   protected void swapPrimaryOwnersWithBackups(Builder builder, int minSegments, int maxSegments) {
   ...
   protected void addBackupOwners(Builder builder) {
      ...
      for (maxSegments = builder.getNumSegments() - 1; maxSegments >= minSegments; maxSegments--) {
         replaceBackupOwners(builder, minSegments, maxSegments);
      }
      ...
      replaceBackupOwners(builder, minSegments + 1, minSegments + 1);
   ...
   protected void replaceBackupOwners(Builder builder, int minSegments, int maxSegments) {
   ...
   public static class Builder {
      ...
      public Builder(int numOwners, int numSegments, List<Address> members) {
         ...
         for (int segment = 0; segment < numSegments; segment++) {
            owners[segment] = new ArrayList<Address>(actualNumOwners);
         }
      ...
      public Builder(Builder other) {
         ...
         List[] owners = new List[numSegments];
         for (int segment = 0; segment < numSegments; segment++) {
            owners[segment] = new ArrayList<Address>(other.owners[segment]);
         }
      ...
      public Address getPrimaryOwner(int segment) {
         return owners[segment].get(0);
      }

      public List<Address> getBackupOwners(int segment) {
         return owners[segment].subList(1, owners[segment].size());
      }

      public OwnershipStatistics getStats() {
         return stats;
      }

      public boolean addOwner(int segment, Address owner) {
         List<Address> segmentOwners = owners[segment];
         if (segmentOwners.contains(owner))
            return false;

         segmentOwners.add(owner);
         stats.incOwned(owner);
         if (segmentOwners.size() == 1) {
            // the first owner
      ...
      public DefaultConsistentHash build(Hash hashFunction) {
         return new DefaultConsistentHash(hashFunction, initialNumOwners, owners.length, members, owners);
      }
