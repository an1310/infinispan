...
public class TopologyAwareConsistentHashFactory extends DefaultConsistentHashFactory {
   ...
   private void addBackupOwnersForLevel(Builder builder, int minSegments, Level level) {
      ...
      for (int currentMax = minSegments; ; currentMax++) {
         if (doAddBackupOwnersForLevel(builder, currentMax, level))
            break;
      }
   }
            ...
            if (builder.getOwned(owner) > maxSegments) {
               ...
               for (Address candidate : builder.getMembers()) {
                  if (builder.getOwned(candidate) < minSegments) {
                     //if (!owners.contains(candidate) && sameLocation(candidate, owner, level)) {
   ...
   private boolean sameLocation(Address candidate, Address replaced, Level level) {
      TopologyAwareAddress topologyAwareCandidate = (TopologyAwareAddress) candidate;
      TopologyAwareAddress topologyAwareOwner = (TopologyAwareAddress) replaced;
      switch (level) {
         case SITE:
            return topologyAwareCandidate.isSameSite(topologyAwareOwner);
         case RACK:
            return topologyAwareCandidate.isSameRack(topologyAwareOwner);
         case MACHINE:
            return topologyAwareCandidate.isSameMachine(topologyAwareOwner);
         case NONE:
            return topologyAwareCandidate.equals(topologyAwareOwner);
      }
      return false;
   }
