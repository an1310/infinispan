...
public class OutboundTransferTask implements Runnable {
   ...
   private void sendEntries(boolean isLast) {
      ...
      if (!chunks.isEmpty()) {
         if (trace) {
            log.tracef("Sending %d cache entries from segments %s of cache %s to node %s", accumulatedEntries, isLast ? segments : entriesBySegment.keySet(), cacheName, destination);
         }
