...
public class StateTransferLockImpl implements StateTransferLock {
   ...
   public void acquireExclusiveTopologyLock() {
      if (trace) {
         log.tracef("Acquiring exclusive topology lock for cache %s", cacheName);
      }
      ownershipLock.writeLock().lock();
      if (trace) {
         log.tracef("Exclusive topology lock successfully acquired for cache %s", cacheName);
      }
   }
   ...
   public void releaseExclusiveTopologyLock() {
      if (trace) {
         log.tracef("Releasing exclusive topology lock for cache %s", cacheName);
      }
   ...
   public void acquireSharedTopologyLock() {
      if (trace) {
         log.tracef("Acquiring shared topology lock for cache %s", cacheName);
      }
      ownershipLock.readLock().lock();
      if (trace) {
         log.tracef("Shared topology lock successfully acquired for cache %s", cacheName);
      }
   }
   ...
   public void releaseSharedTopologyLock() {
      if (trace) {
         log.tracef("Releasing shared topology lock for cache %s", cacheName);
      }
